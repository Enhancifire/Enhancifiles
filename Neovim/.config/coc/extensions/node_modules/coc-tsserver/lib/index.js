...
var TypeScriptReferencesCodeLensProvider = class extends TypeScriptBaseCodeLensProvider {
  ...
  extractSymbol(document, item, parent) {
    ...
    switch (item.kind) {
      case Kind.function: {
        const showOnAllFunctions = import_coc20.workspace.getConfiguration(this.modeId).get("referencesCodeLens.showOnAllFunctions");
...
var TypeScriptRenameProvider = class {
  ...
  async renameFile(fileToRename, newName, token) {
    ...
    edits.documentChanges.push({
      kind: "rename",
      oldUri: import_coc21.Uri.file(fileToRename).toString(),
      newUri: import_coc21.Uri.file(newFilePath).toString(),
      options: {
        overwrite: false,
        ignoreIfExists: true
      }
...
var TypeScriptDocumentSemanticTokensProvider = class {
  ...
  async _provideSemanticTokens(document, requestArg, token) {
    ...
    const builder = new import_coc22.SemanticTokensBuilder();
    let i = 0;
...
var _TagClosing = class {
  constructor(client, descriptionLanguageId) {
    ...
    this._cancel = void 0;
    import_coc23.events.on("InsertCharPre", (character) => {
      this.lastInsert = character;
    }, null, this._disposables);
    import_coc23.events.on("TextChangedI", this.onChange, this, this._disposables);
    import_coc23.events.on("TextChangedP", this.onChange, this, this._disposables);
  }
  async onChange(bufnr, change) {
    let doc = import_coc23.workspace.getDocument(bufnr);
    if (!doc || !doc.attached)
      return;
    ...
    this._timeout = setTimeout(async () => {
      ...
      if (doc.changedtick > changedtick)
        return;
      const position = import_coc23.Position.create(lnum - 1, pre.length);
      const args = Position2.toFileLocationRequestArgs(filepath, position);
      this._cancel = new import_coc23.CancellationTokenSource();
      ...
      if (doc.changedtick === changedtick) {
        import_coc23.snippetManager.insertSnippet(this.getTagSnippet(insertion).value, false, import_coc23.Range.create(position, position));
      }
    }, 50);
  }
  isEnabled(languageId, uri) {
    ...
    if (!import_coc23.workspace.getConfiguration(void 0, uri).get(`${configLang}.autoClosingTags`)) {
      return false;
    }
    return true;
  }
  dispose() {
    ...
    (0, import_coc23.disposeAll)(this._disposables);
    this._disposables = [];
  }
...
var UpdateImportsOnFileRenameHandler = class {
  constructor(client, fileConfigurationManager, languageId) {
    ...
    const watcher = import_coc24.workspace.createFileSystemWatcher(glob);
  ...
  dispose() {
    (0, import_coc24.disposeAll)(this.disposables);
  }
  async doRename(oldResource, newResource) {
    ...
    let oldDocument = import_coc24.workspace.getDocument(oldResource.toString());
    if (oldDocument) {
      await import_coc24.workspace.nvim.command(`silent ${oldDocument.bufnr}bwipeout!`);
    }
    let document = import_coc24.workspace.getDocument(newUri);
    if (document) {
      await import_coc24.workspace.nvim.command(`silent ${document.bufnr}bwipeout!`);
      await wait(30);
    }
    document = await import_coc24.workspace.loadFile(newUri);
    if (!document)
      return;
    await wait(50);
    ...
    if (await this.promptUser(newResource)) {
      await import_coc24.workspace.applyEdit(edits);
    }
  }
  async promptUser(newResource) {
    return await import_coc24.window.showPrompt(`Update imports for moved file: ${newResource.fsPath}?`);
  }
...
var OrganizeImportsCommand = class {
  ...
  async _execute(client, document, sortOnly = false) {
    ...
    if (keys.length == 1) {
      let doc = import_coc25.workspace.getDocument(keys[0]);
      if (doc) {
    ...
    if (edit)
      await import_coc25.workspace.applyEdit(edit);
  }
  async execute(document, sortOnly = false) {
    let client = await this.service.getClientHost();
    if (!document) {
      let doc = await import_coc25.workspace.document;
      if (!doc.attached) {
...
var TsserverService = class {
  constructor(pluginManager) {
    ...
    this.id = "tsserver";
    this.name = "tsserver";
    this.state = import_coc39.ServiceStat.Initial;
  ...
  start() {
    if (this.clientHost)
      return;
    this.state = import_coc39.ServiceStat.Starting;
...
async function activate(context) {
  ...
  const config = import_coc41.workspace.getConfiguration().get("tsserver", {});
  if (!config.enable)
    return;
  ...
  function registCommand2(cmd) {
    let { id, execute } = cmd;
    subscriptions.push(import_coc41.commands.registerCommand(id, execute, cmd));
  }
