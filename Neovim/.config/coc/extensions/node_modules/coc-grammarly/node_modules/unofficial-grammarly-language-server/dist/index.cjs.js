...
var unofficialGrammarlyApi = require('unofficial-grammarly-api');
...
class TextGrammarCheckHost {
    constructor(document, getDocumentContext, getTokenInfo, onError) {
        ...
        this.user = reactivity.ref({ isAnonymous: true, isPremium: false });
        ...
        this.on(unofficialGrammarlyApi.ResponseKind.ALERT, (alert) => this.alerts.value.set(alert.id, alert));
        this.on(unofficialGrammarlyApi.ResponseKind.REMOVE, (alert) => this.alerts.value.delete(alert.id));
        this.on(unofficialGrammarlyApi.ResponseKind.SUBMIT_OT, (message) => {
            var _a;
    ...
    edit(doc_len = this.document.getText().length, nextVersion) {
        ...
        return new unofficialGrammarlyApi.ChangeSet(async (deltas, changes) => {
            var _a;
            if (process.env.NODE_ENV !== 'production')
...
let GrammarlyDiagnosticsService = GrammarlyDiagnosticsService_1 = class GrammarlyDiagnosticsService {
        ...
        this.connection.onCodeAction(async ({ textDocument, context }) => {
            ...
            if (diagnostics.length === 1 && (document === null || document === void 0 ? void 0 : document.host)) {
                ...
                if (alert && alert.replacements.length) {
                    const range = unofficialGrammarlyApi.rebaseTextRange({ start: alert.begin, end: alert.end }, this.findChangesSinceRevision(document, alert.rev));
    ...
    setupDiagnostics(document) {
        ...
        document.host.on(unofficialGrammarlyApi.ResponseKind.SUBMIT_OT, (message) => {
            const history = this.histories.get(document.uri);
    ...
    rebaseWithChanges(document, diagnostic, changes) {
        const range = unofficialGrammarlyApi.rebaseTextRange(diagnostic, changes);
