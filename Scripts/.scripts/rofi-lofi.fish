...
function is_running -a choice
    ...
    if pgrep -f radio-mpv
        ...
        if string match -q -r true (echo $pause_status)
            ...
            set choice_menu_pause $(menu_pause | rofi -dmenu -theme "$HOME"/.config/rofi/config/launcher.rasi -p "$actual_radio Paused, Return/Stop/Nothing?" | cut -d. -f1)
            ...
            set volume_tune $(echo '{ "command": ["get_property", "volume"] }' | socat - /tmp/mpvsocket | jq | grep "data" | awk '{print $2}' | cut -d, -f1)
            set choice_menu_running $(menu_running | rofi -dmenu -theme "$HOME"/.config/rofi/config/launcher.rasi -p "Playing $actual_radio, Stop/Pause/Nothing" | cut -d. -f1)
            switch $choice_menu_running
                ...
                case 2
                    pkill -f radio-mpv
                case 3
                    set volume $(rofi -dmenu -theme "$HOME"/.config/rofi/config/launcher.rasi -p "Volume ($volume_tune%)" -theme-str 'listview {lines: 0;}')
                    echo "{ \"command\": [\"set_property\", \"volume\", $volume] }" | socat - $mpv_socket
                case 4
                    echo ":)"
                case 5
...
function main
    ...
    set choice $(menu | rofi -i -dmenu -theme "$HOME"/.config/rofi/config/launcher.rasi -p "Station" > /tmp/radio_choice_temp; cat /tmp/radio_choice_temp)
    ...
    set volume $(rofi -dmenu -theme "$HOME"/.config/rofi/config/launcher.rasi -p "Volume" -theme-str 'listview {lines: 0;}')
    ...
    switch $choice
        ...
        case 9
            ...
            set URL "https://youtube.com/playlist?list=PL3Xsq8k7CYjMPh-F15n0ClL14jEBr6S37&si=LoaC314DQ3aec97h"
        case 10
