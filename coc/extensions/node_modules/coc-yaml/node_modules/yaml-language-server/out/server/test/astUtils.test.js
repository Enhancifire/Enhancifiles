...
describe('AST Utils Tests', () => {
    ...
    describe('Is Offset in comment', () => {
        it('should detect that offset in comment', () => {
            const doc = testHelper_1.setupTextDocument('#some comment\nfoo: bar');
            const yamlDoc = documents.getYamlDocument(doc);
            const result = astUtils_1.isInComment(yamlDoc.tokens, 4);
            expect(result).to.be.true;
        });
        it('should detect that comment inside object', () => {
            const doc = testHelper_1.setupTextDocument('obj:\n#some comment\n  foo: bar');
            const yamlDoc = documents.getYamlDocument(doc);
            const result = astUtils_1.isInComment(yamlDoc.tokens, 12);
            expect(result).to.be.true;
        });
    });
});
